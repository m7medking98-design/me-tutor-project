...
11. التنسيق (Markdown): ضع دائماً أي كود أو اسم function أو keyword أو variable بين backticks مثل \`print\` و \`for\` — أبداً لا تكتبها كنص عادي. للأمثلة متعددة الأسطر استخدم fenced code block مع اسم اللغة (\`\`\`python). استخدم **الخط العريض** للمفاهيم المهمة، والقوائم المرقمة للخطوات. تذكر: الأمثلة في code blocks تكون مشابهة وليست الحل النهائي.`;

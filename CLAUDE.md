...
**Done — Course restructured into the 8-module PCEP skeleton + Module 1 BUILT (2026-06-14):** User chose to build the whole course **top-down to a top-tier, undoubtable PCEP standard**. The Python course (`c-python`) is now the real 8-module structure in PCEP order (module ids: `m-py-1` ✅, `m-py-vars` ✅, `m-py-decisions`, `m-py-loops`, `m-py-data` ✅, `m-py-functions`, *(m-py-errors — added when built)*, `m-py-projects`). Existing polished lessons kept their ids/slugs and were re-slotted into the correct modules (variables→M2, conditions→M3, loops→M4, functions→M6, final-calculator→M8); demo enrollment seed updated. **Module 1 البداية الصحيحة fully built** (PCEP Block 1: 1.1/1.2/1.5): `what-is-programming` (video), `first-program` (+concept), NEW `print-mastery` workspace (sep/end/escapes), NEW `python-language-map` reference (keywords/comments/indentation, real `body`).

**Done — Module 2 البيانات والمتغيرات BUILT (PCEP Block 1 complete, 2026-06-14):** module `m-py-vars` now 8 lessons (1 video + 5 workspace + 2 reference): NEW video `how-programs-store-data`; `variables` reference gained a full `body` (literals/types/PEP-8); `variables-practice` got a concept brief; NEW workspaces `numbers-and-operators` (int/float, `// % **`, priority, float accuracy, `+=`), `number-systems-and-bitwise` (bin/oct/hex + `bin()/oct()/hex()`, sci-notation, bitwise `& | ^ ~ << >>`), `input-and-casting` (casting + string `* +` + f-strings), `mini-calculator` capstone; NEW reference `types-and-operators-reference` (full `body` tables). **Sandbox constraint discovered:** the Pyodide runtime (`lib/runtime.ts`) has no stdin wired and auto-verify runs code on every Run, so input lessons TEACH real `input()` but SIMULATE the typed value with a labeled preset string (e.g. `raw = "25"`) — keeps the Run→deterministic-output→auto-grade loop intact. With M2 done, **PCEP Block 1 (Modules 1 & 2) + Block 3 (Module 5) now fully covered** (`docs/pcep-coverage.md`). Demo enrollment progress reseeded 29→23 (course grew 24→30 lessons). Video outline: `docs/video-scripts-python-m2.md`. Arabic pending user review.

**RESUME HERE next session: build Module 3 اتخاذ القرار (`m-py-decisions`, PCEP Block 2 obj 2.1 + boolean/relational tail of 1.4)** — extend the existing `conditions` video + `age-gate` lesson (add concept briefs); add: comparisons & logic (`== != < >`, `and`/`or`/`not`, `bool` values, `=` vs `==`), nested & chained `elif` (e.g. grade ladder), and a comparison-operators reference (`body`) with the common `=`/`==` beginner-error table. Then top-down: M4 (loops) → M6 (functions) → M7 (errors, new) → M8 capstones. Each module: build to PCEP standard, add concept briefs to re-slotted lessons, review, commit. Coverage tracked in `docs/pcep-coverage.md`. Work split unchanged: Claude writes workspace lessons + references + video script outlines; user records videos and reviews all Arabic. M5 + M1 + M2 lessons are the quality bar.

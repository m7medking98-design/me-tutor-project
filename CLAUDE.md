...
**RESUME HERE next session: build Module 2 البيانات والمتغيرات (`m-py-vars`, PCEP Block 1: 1.3 + 1.4 + input side of 1.5)** — keep `variables`/`variables-practice` (add concept briefs), add: numbers & operators (int/float, `// % **`, **bitwise**, shortcut `+=`, string `* +`, numeral systems bin/oct/hex, sci-notation, PEP-8 naming), input()+type casting+f-strings, mini-calculator, types reference. Then top-down: M3 → M4 → M6 → M7 (errors, new) → M8 capstones. Each module: build to PCEP standard, add concept briefs to its re-slotted lessons, review, commit. Coverage tracked in `docs/pcep-coverage.md`. Work split: Claude writes workspace lessons (auto-gradable checkpoints) + references (`body` markdown) + video script outlines; user records videos and reviews all Arabic. M5 + M1 lessons are the quality bar.
